...
public class Percolation {
    ...
    public void open(int row, int col) {
        ...
        if (!isOpen(row, col)) {
            ...
            if (row-1 > 0 && isOpen(row-1, col)) { uf.union(rowPosition-nSize + col, rowPosition + col); }
            if (row+1 <= nSize && isOpen(row+1, col)) { uf.union(rowPosition+nSize + col, rowPosition + col); }
            if (col-1 > 0 && isOpen(row, col-1)) { uf.union(rowPosition + col, rowPosition + col-1); }
            if (col+1 <= nSize && isOpen(row, col+1)) { uf.union(rowPosition + col, rowPosition + col+1); }
